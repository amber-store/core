...
- Module `github.com/amber-store/core`; tests run with `go test ./... ` and must stay race-clean (`go test -race`).
...
Add the imports the file needs (`os`, `path/filepath`, `strings`, `github.com/amber-store/core/key`). Note `sealedStore` uses `WithSegmentSize(8<<10)` and `Put`, then `Close` — reopening yields sealed segments (a store closed mid-active leaves that tail active; `Segments` reports only sealed ones).
...
- [ ] **Step 3: Implement.** In `cmd/amber-store/ref.go`, add helpers and rewrite the two actions (imports: add `errors`, `github.com/amber-store/core/gc`, `github.com/amber-store/core/key`):
